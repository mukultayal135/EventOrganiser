...
<head>
    ...
    <script>
        $(document).ready(function() {
            ...
            //saving data through ajax
            $("#update").click(function() { //$("#save").bind("click",function(){                  
                ...
                */
                ...
                if (fun == "none" || selServ.length == 0)
                   ...
                   if (confirm("Are you sure to Update, already entered services will be overwritten ?"))
                       {
                           $.get("ajax-contributor-services-update.php?uid=" + uid + "&functions=" + fun + "&selServices=" + selServ, function(response) {
                        alert(response);
                        //  $("#save").unbind("click");
                        // document.getElementById("selservice").value="";----empty text box on changing function,not on saving, so that contri ko agar dekhna hua, ki konkonsi services save ki hai usne, then not  good to clear on save
        ...
        <div class="row">
            <div class="col-md-12 text-center mt-5">
                <input type="button" id="save" name="save" class="btn btn-danger " value="Save" style="width:70px">
                 <input type="button" id="update" name="update" class="btn btn-danger " value="Update" style="width:75px">
            </div>
        </div>
    </div>
